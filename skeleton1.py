...
# Force fork start method on platforms where spawn is default
...
def dictionary(descriptors, n_clusters):
    ...
    """
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, batch_size=1000, verbose=False)
...
def assignments(descriptors, clusters):
    ...
    matches = bf.knnMatch(descriptors,k=1)
    T, K = descriptors.shape[0], clusters.shape[0]
    ...
    for i, mlist in enumerate(matches):
        best_cluster = mlist[0].trainIdx
        assignment[i, best_cluster] = 1.0
    return assignment
...
def esvm(encs_test, encs_train, C=1000):
    ...
    # set up labels
    # TODO
    # set up labels: one positive + M negatives
    M = encs_train.shape[0]
    y = np.hstack([1, np.zeros(M, dtype=int)])

    def loop(i):
        # compute SVM 
        # and make feature transformation
        # TODO
        # positive sample as first row
        pos = encs_test[i].reshape(1, -1)
        # stack with all negatives
        X = np.vstack([pos, encs_train])
        # train exemplar-SVM
        clf = LinearSVC(C=C, class_weight='balanced', max_iter=10000)
        clf.fit(X, y)
        # extract weight vector and normalize
        x = clf.coef_.ravel()
...
def distances(encs):
    ...
    # encodings
    # TODO
    encs_norm = normalize(encs, axis=1)
    dists = 1.0 - np.dot(encs_norm, encs_norm.T)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('mus.pkl.gz'):
        ...
        # TODO
        mus = dictionary(descriptors=descriptors,n_clusters=100)
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_train = vlad(files_train, mus, args.powernorm, gmp=args.gmp, gamma=args.gamma)
        with gzip.open(fname, 'wb') as fOut:
